...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const typewriterEffect = () => {
      ...
      if (props.text.length > 0 && displayText.value.length < props.text.length) {
        const nextChar = props.text.charAt(displayText.value.length);
        const afterNextChar = props.text.charAt(displayText.value.length + 1);
        const isPartOfSentence = nextChar == ',' || nextChar == ':' || nextChar == ';';
        const isEndOfSentence =
          (nextChar == '.' || nextChar == '!' || nextChar == '?') && (afterNextChar == ' ' || afterNextChar == '\\');
        ...
        if (isStyle) {
          displayText.value +=
            props.text.charAt(displayText.value.length) + (afterNextChar == '*' ? afterNextChar : '');
          ...
          displayText.value += props.text.charAt(displayText.value.length) + afterNextChar;
        } else {
          // Print next character
          displayText.value += props.text.charAt(displayText.value.length);
        }
