...
<template>
  <div class="flex pt-5 items-center gap-y-8 min-h-screen flex-col">
    <FruitBackground />
    <dl class="flex items-center text-sm gap-1 flex-wrap px-5 max-w-xl mx-auto">
      <dt class="font-bold">Jouw mix:</dt>

      <dd class="bg-white shadow-sm rounded-xl p-2" v-for="item in store.questionsAndAnswers">
        {{ item.answer }}
      </dd>
    </dl>
    <!-- {{ store.context }} -->
    <div class="text-lg w-full px-5 max-w-2xl mx-auto">
      <Typewriter v-if="store.context" :text="store.context" @done="showBible = true" />
    </div>
    <transition
